...
<head>
    ...
    <title>Cameră & Detectare Fisuri — BRIDGEGUARD</title>
    ...
    <style>
        ...
        .btn-capture:hover {
            box-shadow: 0 0 40px rgba(74, 142, 255, 0.4), 0 0 80px rgba(74, 142, 255, 0.15);
    ...
    <div class="nav-center">
        <div class="dropdown">
            ...
            <div class="dropdown-menu">
                ...
                <a href="camera.php" style="color:var(--clr-accent);">Cameră</a>
    ...
    <div class="cam-row-top">
        <div class="cam-panel">
            <div class="cam-panel-header">
                ...
                <span class="cam-panel-title">LIVE STREAM</span>
            </div>
            <div class="cam-panel-body" id="livePanel">
                ...
                <div class="cam-placeholder" id="liveOffline">
                    ...
                    Se conectează la cameră...<br>
                    <span style="font-size:0.65rem;opacity:0.6;">Asigură-te că camera_server.py rulează</span>
                </div>
            </div>
        </div>
    </div>
...
const STREAM_URL = `http://${window.location.hostname}:5001/stream`;

const liveImg = document.getElementById('liveStream');
liveImg.src = STREAM_URL;
...
async function captureAndDetect() {
    ...
    setStatus('Se face fotografia și se trimite la analiza AI...', 'info');

    try {
        const resp = await fetch('api_camera.php?action=detect', { method: 'POST' });
        const data = await resp.json();

        if (!data.success) {
            throw new Error(data.error || 'Eroare server cameră');
        }
        ...
        if (!document.getElementById('capturedImg').src) {
            document.getElementById('capturePlaceholder').style.display = '';
        }
        if (!document.getElementById('detectedImg').src) {
