...
RPI5_IP = '**'          
RPI5_URL = f'http://{RPI5_IP}:5005'                 
TEMP_URL = f'http://{RPI5_IP}:5005/temperatura'     
...
def actualizeaza_temperatura():
    """Citeste MAX6675 si trimite la RPi (port 5005). Throttled la 2s, nu blocheaza semaforul."""
    ...
    verifica_wifi()
    try:
        r = urequests.post(TEMP_URL, json={'temperatura': temp}, timeout=3)
        r.close()
...
def verifica_wifi():
    """Reconecteaza daca s-a pierdut conexiunea intre timp."""
    global wlan
...
def trimite_cu_retry(url, payload, max_incercari=3, pauza=1):
    for incercare in range(1, max_incercari + 1):
        try:
            r = urequests.post(url, json=payload, timeout=3)
            raspuns = r.text
            r.close()
            return raspuns
        except Exception as e:
...
def trimite_stare(culoare, greutate=0.0, mesaj=''):
    """Trimite starea curenta catre RPi 5 (nu blocheaza mult timp daca esueaza)"""
...
def heartbeat_semafor(greutate=None):
    """Retrimite starea curenta daca au trecut >3s de la ultima trimitere (tine site-ul online)."""
...
def cere_poza_si_analiza():
    """Ii spune lui RPi 5 sa ceara poza de la ESP32-CAM si sa o analizeze cu Claude"""
    verifica_wifi()
    raspuns = trimite_cu_retry(RPI5_URL + '/fa-poza', {})
    if raspuns:
