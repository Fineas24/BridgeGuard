...
WIFI_SSID = "Pico"
WIFI_PASS = "bagiparolasidaienter"
...
def citeste_ultrasonic():
    """Returneaza distanta in cm, sau -1.0 la timeout (>30ms = >5m)."""
    TRIG.low()
    utime.sleep_us(2)
    TRIG.high()
    utime.sleep_us(10)
...
def citeste_vibratie():
    """Intensitate 0-1000 cu coborare lina dupa impuls."""
...
def bobinaj_intact():
    """True = cablu intreg (circuit inchis), False = cablu rupt."""
    return BOBINAJ.value() == 0

def trimite_date(payload):
    """POST manual pe raw socket, cu timeout explicit (nu depinde de urequests)."""
