...
<head>
    ...
    <title>Cameră #2 — BRIDGEGUARD</title>
    ...
    <div class="nav-center">
        <div class="dropdown">
            ...
            <div class="dropdown-menu">
                ...
                <a href="camera_esp2.php" style="color:var(--clr-accent);">Camera #2</a>
...
<main class="container">
    ...
    <div class="cam-hero">
        <h1>CAMERA #2 — ESP32-CAM</h1>
        <p>Vizualizare live · Captura foto · Analiza AI fisuri structurale</p>
    </div>
    ...
    <div class="cam-row-top">
        <div class="cam-panel">
            <div class="cam-panel-header">
                ...
                <span class="cam-panel-title">LIVE STREAM — CAMERA #2</span>
            </div>
            <div class="cam-panel-body" id="livePanel">
                ...
                <div class="cam-placeholder" id="liveOffline">
                    ...
                    Se conecteaza la Camera #2...<br>
                    <span style="font-size:0.65rem;opacity:0.6;">Verifica daca camera este pornita si conectata la WiFi</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="cam-row-bottom">
        <div class="cam-panel">
            <div class="cam-panel-header">
                ...
                <span class="cam-panel-title">FOTOGRAFIE CAPTURATA</span>
            </div>
            <div class="cam-panel-body" id="capturePanel">
                <img id="capturedImg" src="" alt="Captura" style="display:none;">
                ...
                <div class="cam-placeholder" id="capturePlaceholder">
                    <div class="cam-placeholder-icon">📷</div>
                    Apasa butonul pentru a face o fotografie
                </div>
            </div>
        ...
        <div class="cam-panel">
            ...
            <div class="cam-panel-body" id="detectPanel" style="position:relative;">
                ...
                <div class="cam-loading" id="detectLoading">
                    <div class="spinner"></div>
                    <span class="spinner-text">Se analizeaza cu AI...</span>
                </div>
                <div class="cam-placeholder" id="detectPlaceholder">
                    <div class="cam-placeholder-icon">🔍</div>
                    Rezultatul analizei AI va aparea aici
                </div>
...
const ESP_STREAM_URL = 'http://192.168.100.218:81/stream';
...
async function captureAndDetect() {
    ...
    setStatus('Se face fotografia si se trimite la analiza AI...', 'info');

    try {
        const resp = await fetch('api_camera_esp2.php?action=detect', { method: 'POST' });
        const data = await resp.json();

        if (!data.success) {
            throw new Error(data.error || 'Eroare la captura camerei');
        }
        ...
        if (data.detection_failed) {
            ...
            setStatus('Eroare la analiza AI — verifica consola pentru detalii.', 'error');
            ...
            setStatus(`S-au detectat ${data.count} fisur(i) in imagine!`, 'warning');
        } else {
            badge.textContent = 'FARA FISURI';
            badge.className = 'detection-result clean';
            document.getElementById('detectResult').textContent = 'Nicio fisura detectata';
            setStatus('Analiza completa — nicio fisura detectata.', 'success');
        }
    ...
    btn.disabled = false;
    btn.textContent = 'CAPTURA & DETECTARE FISURI';
}
